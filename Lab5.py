...
def quaternion_rot(tetta, t_x, t_y, t_z):
    ...
    for i in range(len(v)):
        ...
        # ВАЖНО: ВОЗМОЖНА ПРОБЛЕМА С ИНДЕКСОМ (больше нет)
...
def patriotism_model(t_x, t_y, t_z, tetta, name, rot_type): # иде не нравилось otrisovat_model
    parser(name)
    if rot_type == 1:
        quaternion_rot(tetta, t_x, t_y, t_z)
    else:
...
def paint_model(t_x, t_y, t_z, tetta, name_obj, name_img, rot_type):
    ...
    patriotism_model(t_x, t_y, t_z, tetta, name_obj, rot_type) # Не удалять!!!! Это важный вызов всего, чего мы добились!!


# смещение: t_x, t_y, t_z
# поворот для кватернионов: teta ()
paint_model(0.25, -0.04, 0.35, -1, 'obj/'+'model_1'+'.obj',   'tex/'+'bunny-atlas'+'.jpg', 1)
paint_model(-0.24, -0.04, 0.22, 0.5, 'obj/'+'model_1'+'.obj', 'img'+'.png', 2)
paint_model(-0.07, 10.04, 70.0, 1, 'obj/'+'12221_Cat_v1_l3'+'.obj',  'tex/'+'Cat_diffuse'+'.jpg', 1)
paint_model(-2.17, 0.04, 6, -1, 'obj/'+'model'+'.obj',            'tex/'+'model'+'.bmp', 1)
